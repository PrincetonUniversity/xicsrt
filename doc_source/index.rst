...
`json` or `yml`), output is a python dictionary (which can be saved as `hdf5`).
...
wrappers. Than being said, XICSRT can be used interactively using a `jupyter`_
notebook and some familiarity with `numpy`_, `matplotlib`_ and `plotly`_.
