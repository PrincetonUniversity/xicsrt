...
class XicsrtOpticCone(XicsrtOpticCrystal):
    ...
    def standard_cone(self, a, b, inp, return_XQ_hat=False):
        ...
        SD0 = D0 - S
        SD0_hat = SD0 / jnp.linalg.norm(SD0)
        aD0SC = vector_angle(SD0_hat, -1 * CS_hat)

        CP_hat = -1 * jnp.cross(SD0_hat, C0_yaxis)
        CP_dist = CS_dist * jnp.sin(aD0SC)
        P = C + CP_hat * CP_dist
        ...
        XP_hat = vector_rotate(CP_hat, SD0_hat, b)
        X = P - XP_hat * CP_dist

        XQ_hat = vector_rotate(CQ_hat, SD0_hat, b)
        if return_XQ_hat:
            return X, XQ_hat
        else:
            return X

    def standard_cone_jax(self, a, b, inp):
        xyz = self.standard_cone(a, b, inp)
        profiler.start('standard_cone_jax, normal')
        dfda, dfdb = jax.jacfwd(self.standard_cone, (0, 1))(a, b, inp)
        norm_ad = np.cross(dfda, dfdb)
        norm_ad /= np.linalg.norm(norm_ad)
        profiler.stop('standard_cone_jax, normal')
        return xyz, norm_ad
        ...
        for ii_a in range(num_a):
            for ii_b in range(num_b):
                ...
                if self.param['normal_method'] == 'jax':
                    xyz, norm = self.standard_cone_jax(a, b, inp)
                elif self.param['normal_method'] == 'ideal_np':
                    xyz, norm = self.standard_cone(a, b, inp, return_XQ_hat=True)
                else:
