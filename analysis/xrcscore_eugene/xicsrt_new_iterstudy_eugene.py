# -*- coding: utf-8 -*-
"""
Created on Thu Apr 03 09:48:30 2020
...
def get_config():
    ...
    set print_results to False to disable console feedback
    change the random seed to alter the random numbers generated
    
    setting keep_meta, keep_images, and keep_history to False prevents the ray
    tracer from recording metadata, ray impact images, and ray history. This 
    can speed up raytrace runs if those outputs are unnecessary.
    
    setting save_meta, save_images, and save_history to False prevents the ray
    tracer from saving files containing metadata, ray impact images, and ray
    history. 
    
    If save_meta, save_images, and save_history are set to True, but keep_meta,
    keep_images, and keep_history are set to False, then the ray tracer will
    output empty files.
    """
    ...
    Setting 'use_poisson' to True will cause the plasma bundles to emit rays
    according to Poisson statistics, rather than using exact integer intensities.
    This is useful for preserving ray statistics.
    Setting 'do_monochrome' to True forces the plasma to emit at one exact wavelength
    ...
    """Geometry settings
    The plasma takes the geometry of the given WOUT file, centered at [0,0,0]
    Only a small cubic chunk of the plasma is rendered and emits rays
    This cubic chunk has a 'width', 'height', and 'depth' (meters)
    It also has a position 'origin', normal 'zaxis', and orientation 'xaxis' (meters)
    The ray bundles generated by the plasma will aim at the given 'target' (meters)
    """
    ...
    config['sources']['plasma']['time_resolution']  = 1e-6
    config['sources']['plasma']['spread']           = 0.2
    config['sources']['plasma']['mass_number']      = 131.293
    config['sources']['plasma']['wavelength']       = 2.7203 #2.19 
    ...
    """Type and file settings
    setting 'do_bragg_check' to False makes the crystal ignore Bragg conditions
    setting 'do_miss_check'  to False prevents the crystal from masking missed rays
    setting 'use_meshgrid'   to False defaults to using simple rectangle geometry
    ...
    """
    config['optics']['crystal']['crystal_spacing']  = 1.7059 #1.42 
    ...
    """Geometry Settings
    crystal 'width' and 'height' (meters) must envelop the entire meshgrid
    ...
    config['optics']['crystal']['radius']          = 2.400 #2.200 
    ...
    """Type and file settings
    setting 'do_bragg_check' to False makes the graphite ignore Bragg conditions
    setting 'do_miss_check'  to False prevents the graphite from masking missed rays
    setting 'use_meshgrid'   to False defaults to using simple rectangle geometry
    possible rocking curve types include 'STEP', 'GAUSS', and 'FILE'
    sigma and pi are polarized rocking curves. 'rocking_mix' interpolates between them.
    A 'rocking_mix' of 1.0 is 100% sigma curve, while 0.0 is 100% pi curve.
    ...
    """Geometry Settings
    graphite 'width' and 'height' (meters) must envelop the entire meshgrid
    """
    ...
    config['optics']['graphite']['width']             = 0.160
    config['optics']['graphite']['height']            = 0.160
    ...
    """
    setting 'do_miss_check'  to False prevents the detector from masking missed rays
    'pixel_size', 'width', and 'height' are in (meters)
    """
    ...
    #config = setup_core_scenario(config, crystal_corners, detector_centers, chord)
...
def setup_core_scenario(config, c_corners, d_centers, chord):
    """
    Set up a scenario with only the spherical crystal, pilatus detector, and a
    direct view of the plasma
    """
    ## Unpack variables and convert to meters
    c_corners  = c_corners[chord]       / 1000
    d_centers  = d_centers[chord]       / 1000

    #calculate geometric properties of all optics
    c_basis      = np.zeros([3,3], dtype = np.float64)
    d_basis      = np.zeros([3,3], dtype = np.float64)
    ...
    #calculate the germanium spherical crystal's sightline of the plasma
    #start with the detector-crystal vector, normalize, and reflect it
    sightline    = c_origin - d_origin
    sightline   /= np.linalg.norm(sightline)
    sightline   -= 2 * np.dot(sightline, c_basis[2,:]) * c_basis[2,:]
    
    ## Repack variables
    #config['sources']['focused']['target']        = g_origin
    config['sources']['plasma']['target']         = c_origin
    config['filters']['sightline']['origin']      = c_origin
    config['filters']['sightline']['direction']   = sightline
